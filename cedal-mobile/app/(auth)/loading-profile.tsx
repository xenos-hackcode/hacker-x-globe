// app/(auth)/loading-profile.tsx  ← general profile loader
...
const PHASES = [
  ...
  "Syncing profile vectors...",
  "Validating security matrix...",
  "Deploying identity profile...",
...
useEffect(() => {
  if (phase === PHASES.length - 1) {
    const t = setTimeout(async () => {
      try {
        const passcodeDone = await SecureStore.getItemAsync(
          "cedal_passcode_done"
        );

        if (!passcodeDone) {
          // first time after sign-up → node passcode flow
          router.replace("/(auth)/(member)/create-passcode" as any);
        } else {
          // subsequent times → go into member group, which redirects to /home
  ...
  const orbStyle = useAnimatedStyle(() => ({
    opacity: 0.25 + 0.4 * orbPulse.value,
      ...
      <Animated.View style={[styles.panelOuter, panelAnimatedStyle]}>
        <View style={styles.panelInner}>
          ...
          <View style={styles.logoBlock}>
            ...
            <Text style={styles.logoTitle}>CEDAL / PROFILE NODE</Text>
            <Text style={styles.logoSubtitle}>IDENTITY DEPLOYMENT</Text>
          </View>
          ...
          <View style={styles.statusBlock}>
            <Text style={styles.statusLabel}>
              ...
              <Text style={styles.statusValue}>
                {phase < PHASES.length - 1 ? "DEPLOYING" : "READY"}
              </Text>
            </Text>
          ...
          <View style={styles.logsShell}>
            ...
            <View style={styles.logsList}>
              {[
                { text: "Node handshake", minPhase: 1 },
                { text: "Profile vectors deployed", minPhase: 2 },
                { text: "Security matrix validated", minPhase: 4 },
          ...
          <View style={styles.regionBlock}>
            <Text style={styles.regionText}>
              REGION: LON-01  ·  CHANNEL: PROFILE-DEPLOY-01
            </Text>
          </View>
        </View>
      </Animated.View>
